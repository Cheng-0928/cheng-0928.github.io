...
var UserDetail = new function () {
    ...
    self.show = function (user_id) {
        ...
        $.when(
            ...
            $.ajax({
                url: Config.get_subchanges_url(),
                dataType: "json"
            })
        ).done(function (submissions_result, subchanges_result) {
            self.submissions_callback(submissions_result[0], subchanges_result[0]);
        }).fail(function () {
    ...
    self.submissions_callback = function (submissions_data, subchanges_data) {
        self.submissions = new Object();
        for (var t_id in DataStore.tasks) {
            self.submissions[t_id] = new Array();
        }

        var changes_by_submission = new Object();
        for (var change_id in subchanges_data) {
            var change = subchanges_data[change_id];
            changes_by_submission[change["submission"]] = change;
        }

        for (var s_id in submissions_data) {
            var submission = submissions_data[s_id];
            if (submission["user"] != self.user_id) {
                continue;
            }

            var change = changes_by_submission[s_id] || {};
            submission["score"] = change["score"] || 0;
            submission["extra"] = change["extra"] || [];
            submission["token"] = change["token"] || false;
            self.submissions[submission['task']].push(submission);
        }

        for (var task_id in self.submissions) {
            self.submissions[task_id].sort(function (a, b) {
